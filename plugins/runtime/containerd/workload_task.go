...
//nolint:cyclop,funlen // Start owns the task, CNI rollback, and restart-monitor transaction boundary.
func (backend *nativeWorkloadBackendV1) Start(ctx context.Context, identifier string) (resultErr error) {
	...
	if err != nil {
		preserveNetwork = preserveNetwork || backend.startMayHaveTakenEffect(ctx, identifier)
...
func (backend *nativeWorkloadBackendV1) startMayHaveTakenEffect(
	ctx context.Context,
	identifier string,
) bool {
	probeContext, cancel := context.WithTimeout(context.WithoutCancel(ctx), containerdRequestTimeout)
	defer cancel()

	lifecycle, found, err := backend.taskLifecycle(probeContext, identifier)
	if err != nil {
		return true
	}

	return found && lifecycle != application.WorkloadLifecycleCreated
}
