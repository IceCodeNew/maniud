...
func (server *sshTestServer) serveConnection(connection net.Conn) {
	...
	for channel := range channels {
		server.channels <- channel.ChannelType()
...
//nolint:cyclop,funlen // The fixture owns listener, connection, and agent goroutine cleanup branches.
func startSSHAgent(t *testing.T, identity sshTestIdentity) string {
	t.Helper()

	directory, err := os.MkdirTemp( //nolint:usetesting // A short path avoids Darwin's Unix socket length limit.
		filepath.Join(string(os.PathSeparator), "tmp"),
		"maniud-agent-",
	)
	if err != nil {
		t.Fatalf("os.MkdirTemp(SSH agent) error = %v", err)
	}

	t.Cleanup(func() {
		removeErr := os.RemoveAll(directory)
		if removeErr != nil {
			t.Errorf("os.RemoveAll(SSH agent) error = %v", removeErr)
		}
	})

	listener := newSSHTestListener(t, "unix", filepath.Join(directory, "agent.sock"))
