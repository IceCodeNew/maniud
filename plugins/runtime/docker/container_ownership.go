...
func decodeContainerOwnership(
	labels map[string]string,
	imageReference string,
	imageTarget domain.Digest,
	platformManifest domain.Digest,
) (domain.WorkloadOwnership, domain.Digest) {
	ownership := decodeOwnership(labels, imageTarget, platformManifest)
	if ownership.Status != domain.OwnershipConflicting {
		return ownership, imageTarget
	}

	reference, err := imageref.Parse(imageReference)
	if err != nil {
		return ownership, imageTarget
	}
	labeledConfig, err := domain.ParseDigest(labels[domain.LabelImageConfigDigest])
	if err != nil {
		return ownership, imageTarget
	}
	if imageTarget != reference.Digest() {
		return ownership, imageTarget
	}

	ownership = decodeOwnership(labels, labeledConfig, platformManifest)
	if ownership.Status != domain.OwnershipManaged || ownership.Reference != reference.Digest() {
		var conflicting domain.WorkloadOwnership

		return conflicting, imageTarget
	}

	// Docker's containerd image store reports the immutable reference target in
	// InspectResponse.Image. The manifest descriptor and resolved ownership
	// labels transitively bind that target to the image configuration digest.
	return ownership, labeledConfig
}
