...
func isRegistry(value string) bool {
	return strings.ContainsAny(value, ".:") || strings.EqualFold(value, "localhost")
}

func ambiguousRegistry(value string) bool {
	first, _, found := strings.Cut(value, "/")

	return found && strings.ToLower(first) != first && !isRegistry(first)
}

func validDigest(value digest.Digest) bool {
	return value.Validate() == nil && value.Algorithm() == digest.SHA256 &&
