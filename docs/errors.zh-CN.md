...
| `operation_cancelled` | Signal 或调用方 context 取消了操作。 | 再次执行同一条命令，让 maniud 恢复未完成事务。 |
...
Debug 结果有长度限制，并会移除环境变量值、包含凭据的 URL、authorization header、secret assignment 和 private key。
