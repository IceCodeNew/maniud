...
`d` opens a read-only Details page for the selected field. Details renders the
complete canonical Current and Proposed values, wraps them to the body width,
and scrolls vertically. It does not use ellipsis or horizontal scrolling and
contains no mutation control. `d` or `Esc` returns to Review. A stale projection
closes Details and reloads Review before another action can run.
...
| Blocked or failed | Red | Red | `[x]` and state text |
