...
//nolint:cyclop // The assertion verifies every independently mapped rich field.
func TestEncodeAndDecodeInspectRichConfiguration(t *testing.T) {
	t.Parallel()
	...
	want.Ports[0].HostIP = ""
	want.Tmpfs[0].Options = []string{"size=64m", "mode=1777"}
...
func TestCanonicalOwnsInputAndNormalizesRuntimeDefaults(t *testing.T) {
	...
	spec.Tmpfs = []containerconfig.TmpfsMount{{
		Target: "/tmp", Options: []string{"size=1m", propagationPrivate, "nosuid", "nodev", "tmpcopyup"},
	}}
...
func TestEquivalentHandlesLibpodRuntimeDefaults(t *testing.T) {
	...
	expected = minimalSpec()
	expected.Ulimits = []containerconfig.Ulimit{{Name: "nofile", Soft: 1024, Hard: 2048}}
	observed = expected.Clone()
