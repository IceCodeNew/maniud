...
func TestEncodeDecodePreservesNoCopyAnonymousVolume(t *testing.T) {
	t.Parallel()

	spec := portableSpec()
	spec.Mounts = []containerconfig.Mount{{
		Kind: containerconfig.MountVolume, Target: dockerTestDataTarget,
	}}
	request, err := Encode(spec, CreateOptions{ImageReference: testImage})
	if err != nil {
		t.Fatalf("Encode() error = %v", err)
	}
	observed, err := Decode(spec.ContainerName, request.Config, request.HostConfig)
