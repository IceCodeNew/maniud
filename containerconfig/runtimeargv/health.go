...
// durationPattern follows the duration grammar shared by supported runtime clients.
...
func canonicalHealthValue(field, value string) (string, error) {
	...
	case healthInterval, healthStartInterval, healthStartPeriod, healthTimeout:
		...
		if len(value) <= maximumTextLength && durationPattern.MatchString(value) && err == nil && duration > 0 {
			return value, nil
		}
	}
