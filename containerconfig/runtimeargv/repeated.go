...
//nolint:cyclop,funlen // Dispatch keeps each repeated runtime option bound to one typed validator.
func (parser *argvParser) addRepeated(field, value string) error {
	...
	case repeatedExpose:
		return parser.addExposedPort(value)
	case repeatedLabel:
...
func parseDevice(value string) (containerconfig.DeviceMapping, error) {
	parts := strings.Split(value, ":")
	mapping := containerconfig.DeviceMapping{Permissions: defaultDevicePermissions}
	switch len(parts) {
	case 1:
	...
	default:
		return containerconfig.DeviceMapping{}, ErrInvalid
	}
...
func parseUlimit(name, value string) (containerconfig.Ulimit, error) {
	...
	if err != nil {
		return containerconfig.Ulimit{}, err
	}
	...
	if soft != hard && (soft == -1 || (hard != -1 && soft > hard)) {
		return containerconfig.Ulimit{}, ErrInvalid
	}

	return containerconfig.Ulimit{Name: name, Soft: soft, Hard: hard}, nil
}
...
func (parser *argvParser) addExposedPort(value string) error {
	port, err := parseExposedPort(value)
	if err != nil {
		return err
	}
	if !slices.Contains(parser.service.ExposedPorts, port) {
		parser.service.ExposedPorts = append(parser.service.ExposedPorts, port)
	}
