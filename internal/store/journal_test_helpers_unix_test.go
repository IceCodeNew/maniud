...
const testUnknownValue = "unknown"
...
func resolveJournal(
	...
	applied, err := lock.CommitAppliedService(context.Background(), record.ID, testAppliedServiceIntent())
	requireNoError(t, err)
	if applied.TransactionID != record.ID {
		t.Fatalf("applied service = %#v", applied)
	}

	reusedApplied, err := lock.CommitAppliedService(context.Background(), record.ID, testAppliedServiceIntent())
	requireNoError(t, err)
	if reusedApplied != applied {
		t.Fatalf("reused applied service = %#v", reusedApplied)
	}

	record, err = state.Transaction(context.Background(), record.ID)
	requireNoError(t, err)
...
func testAppliedServiceIntent() AppliedServiceIntent {
	return AppliedServiceIntent{
		WorkloadID:             "maniud-workload",
		ConfigurationDigest:    domain.Hash([]byte("runtime configuration")),
		StorageDigest:          domain.Hash([]byte("runtime storage")),
		ReferenceDigest:        domain.Hash([]byte("resolved image reference")),
		PlatformManifestDigest: domain.Hash([]byte("platform image manifest")),
		ImageConfigDigest:      domain.Hash([]byte("image configuration")),
	}
}
...
func assertTransaction(
	...
	if transaction.ID == (TransactionID{}) || transaction.Kind != intent.Kind || transaction.State != state ||
