...
//nolint:cyclop // One linear traversal owns every descriptor and creation race.
func ensureStateDirectory(ctx context.Context, path string) error {
	if !validStatePath(path) {
	...
	for _, component := range splitAbsolutePath(filepath.Dir(path)) {
		next, nextErr := unix.Openat(
			descriptor,
			component,
		...
		created := false
		if valid && errors.Is(nextErr, unix.ENOENT) {
			mkdirErr := unix.Mkdirat(descriptor, component, privateDirectoryMode)
			valid = mkdirErr == nil || errors.Is(mkdirErr, unix.EEXIST)
			if valid {
				next, nextErr = unix.Openat(
					descriptor,
					component,
		...
		descriptor = next
		valid = valid && nextErr == nil && (!created || privateDirectoryDescriptor(next))
		if !valid {
			break
		}
	}
...
func privateDirectoryDescriptor(descriptor int) bool {
	identity, valid := descriptorIdentity(descriptor)

	return valid && identity.mode&unix.S_IFMT == unix.S_IFDIR &&
		int64(identity.owner) == int64(unix.Geteuid()) && identity.mode&0o077 == 0
}
