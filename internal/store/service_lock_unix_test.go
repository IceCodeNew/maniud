...
func TestServiceLockWaitHonorsCancellation(t *testing.T) {
	t.Parallel()
	...
	contender, err := state.LockService(ctx, "project", "api")
	if contender != nil || !errors.Is(err, context.Canceled) || !errors.Is(err, ErrUnavailable) {
		t.Fatalf("LockService(cancelled) = %#v, %v", contender, err)
	}
}
...
func TestServiceLockRejectsReplacementDuringAcquisition(t *testing.T) {
	...
	err := serviceLock.acquire(context.Background(), false)
...
func TestServiceLockNilReceiver(t *testing.T) {
	...
	if !errors.Is((*ServiceLock)(nil).acquire(context.Background(), false), ErrUnavailable) {
