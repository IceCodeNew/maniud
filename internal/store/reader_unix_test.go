...
func TestOpenReaderTreatsMissingStateAsEmptyWithoutCreatingFiles(t *testing.T) {
	...
	for _, path := range paths {
		...
		if err != nil || found || transaction != empty {
...
func TestOpenReaderAndStoreExcludeStartupMigration(t *testing.T) {
	t.Parallel()
...
func TestOpenReaderRejectsInvalidStateWithoutRepair(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "backup without database",
			setup: func(t *testing.T, path string) {
				t.Helper()
				requireNoError(t, os.WriteFile(path+".schema-orphan", nil, privateFileMode))
			},
			want: ErrInvalidState,
		},
		{
		...
		{
			name: "pending migration backup",
			setup: func(t *testing.T, path string) {
				t.Helper()
				state := openJournalStore(t, path)
				requireNoError(t, state.Close())

				plan, valid := planMigrationBackup(filepath.Base(path), 2, 3, 1, [32]byte{1})
				if !valid {
					t.Fatal("planMigrationBackup() rejected fixture")
				}

				requireNoError(t, os.WriteFile(filepath.Join(filepath.Dir(path), plan.artifactName), []byte{1}, 0o600))
				requireNoError(t, os.WriteFile(filepath.Join(filepath.Dir(path), plan.manifestName), plan.content, 0o600))
			},
			want: ErrUnavailable,
		},
		{
			name: "malformed migration entry",
			setup: func(t *testing.T, path string) {
				t.Helper()
				state := openJournalStore(t, path)
				requireNoError(t, state.Close())
				requireNoError(t, os.WriteFile(path+".schema-malformed", nil, 0o600))
			},
			want: ErrInvalidState,
		},
		{
			...
			setup: func(t *testing.T, path string) {
				...
				_, err = state.database.ExecContext(context.Background(), "UPDATE schema_version SET version = 2")
				requireNoError(t, err)
				requireNoError(t, state.Close())
			},
			want: ErrInvalidState,
...
func TestReaderContainsInvalidRequestsAndClosedResources(t *testing.T) {
	t.Parallel()
...
func TestReaderContainsSQLiteAndIdentityFailures(t *testing.T) {
	...
	_, _, err := rolledBack.UnresolvedTransaction(context.Background(), "project", "api")
	if !errors.Is(err, ErrInvalidState) {
		t.Fatalf("UnresolvedTransaction(rolled back) error = %v", err)
	}

	_, err = rolledBack.Actions(context.Background(), TransactionID{})
	if !errors.Is(err, ErrInvalidState) {
		t.Fatalf("Actions(rolled back) error = %v", err)
	}
	...
	_, _, err = replaced.UnresolvedTransaction(context.Background(), "project", "api")
	if !errors.Is(err, ErrInvalidState) {
		t.Fatalf("UnresolvedTransaction(replaced) error = %v", err)
	}

	_, err = replaced.Actions(context.Background(), TransactionID{})
	if !errors.Is(err, ErrInvalidState) {
		t.Fatalf("Actions(replaced) error = %v", err)
	}
...
func TestReaderInternalGuardsClassifyFailures(t *testing.T) {
	...
	if !errors.Is((*stateAnchor)(nil).requireReaderValid(false), ErrInvalidState) ||
		!errors.Is((*Reader)(nil).requireValid(), ErrInvalidState) {
	...
	if !errors.Is(rejectMigrationBackupForReader(nil, false), ErrInvalidState) ||
		!errors.Is(rejectMigrationBackupNames(nil, false, nil), ErrInvalidState) {
		t.Fatal("migration reader accepted a nil anchor")
	}

	invalidAnchor := newStateAnchor(-1, "", "state.db")
	if !errors.Is(rejectMigrationBackupForReader(invalidAnchor, false), ErrInvalidState) ||
		!errors.Is(rejectMigrationBackupNames(invalidAnchor, false, nil), ErrInvalidState) {
		t.Fatal("migration reader accepted an invalid anchor")
	}
...
func TestReaderInternalFaultBoundaries(t *testing.T) {
	...
	err = rejectMigrationBackupForReaderWithReadDir(
		anchor,
		false,
		func(string) ([]os.DirEntry, error) { return nil, errJournalTest },
	)
	if !errors.Is(err, ErrUnavailable) {
		t.Fatalf("rejectMigrationBackupForReaderWithReadDir() = %v", err)
	}
