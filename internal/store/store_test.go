...
func TestOpenCreatesMissingPrivateStateDirectories(t *testing.T) {
	t.Parallel()

	root := privateTempDir(t)
	directory := filepath.Join(root, ".local", "state", "maniud")
	path := filepath.Join(directory, "state.db")

	state, err := Open(context.Background(), path)
	if err != nil {
		t.Fatalf("Open(missing directories) error = %v", err)
	}

	for current := directory; current != root; current = filepath.Dir(current) {
		info, statErr := os.Stat(current)
		if statErr != nil || !info.IsDir() || info.Mode().Perm() != 0o700 {
			t.Fatalf("created state directory %q = %#v, %v", current, info, statErr)
		}
	}
...
func TestOpenDoesNotCreateStateThroughSymlink(t *testing.T) {
	t.Parallel()

	root := privateTempDir(t)
	target := privateTempDir(t)
	requireNoError(t, os.Symlink(target, filepath.Join(root, "state")))

	state, err := Open(context.Background(), filepath.Join(root, "state", "maniud", "state.db"))
	if state != nil || !errors.Is(err, ErrInvalidState) {
		t.Fatalf("Open(symlink parent) = %#v, %v", state, err)
	}
	anchor, anchorErr := openStateAnchor(
		context.Background(),
		filepath.Join(root, "state", "maniud", "state.db"),
	)
	if anchor != nil || !errors.Is(anchorErr, ErrInvalidState) {
		t.Fatalf("openStateAnchor(symlink parent) = %#v, %v", anchor, anchorErr)
	}

	_, statErr := os.Stat(filepath.Join(target, "maniud"))
	if !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("Open(symlink parent) created target state: %v", statErr)
	}
}
...
func TestClassifySQLiteProbePreservesContention(t *testing.T) {
	t.Parallel()

	err := classifySQLiteProbe(context.Background(), sqliteResultError(sqliteResultBusy))
	if !errors.Is(err, ErrUnavailable) {
		t.Fatalf("classifySQLiteProbe(busy) error = %v", err)
	}
}

type sqliteResultError int

func (err sqliteResultError) Error() string {
	return "SQLite result"
}

func (err sqliteResultError) Code() int {
	return int(err)
}
