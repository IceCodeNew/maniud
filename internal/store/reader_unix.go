...
// transaction state. Close releases the SQLite snapshot and migration lock.
type Reader struct {
	database    *sql.DB
	transaction *sql.Tx
	anchor      *stateAnchor
...
// OpenReader opens current durable state without creating files, migrating a
// schema, recovering a backup, or changing application state. A missing state
// database is represented by an empty Reader.
...
func readerDirectoryHasNoState(
	...
	entries, err := readDir(platformEntryPath(anchor, "."))
	if err != nil {
	...
	_, _, found, err := migrationBackupNames(names, anchor.databaseName)
	if err != nil || found {
		return false, ErrInvalidState
	}
...
func finishOpenReader(ctx context.Context, anchor *stateAnchor) (*Reader, error) {
	...
	err := rejectMigrationBackupForReader(anchor, sidecars)
	if err != nil {
		return nil, err
	}
...
func validateReaderSnapshot(ctx context.Context, reader *Reader) error {
	...
	err = validateSchema(ctx, reader.transaction, currentSchemaVersion)
	if err != nil {
		return err
	}
...
func validReaderSettings(ctx context.Context, query schemaQueryer, sidecars bool) bool {
	var (
		foreignKeys int
		journalMode string
		queryOnly   int
...
func rejectMigrationBackupForReader(anchor *stateAnchor, sidecars bool) error {
	if anchor == nil {
		return ErrInvalidState
	}

	err := anchor.requireReaderValid(sidecars)
	if err != nil {
		return err
	}

	return rejectMigrationBackupForReaderWithReadDir(anchor, sidecars, os.ReadDir)
}

func rejectMigrationBackupForReaderWithReadDir(
	anchor *stateAnchor,
	sidecars bool,
	readDir func(string) ([]os.DirEntry, error),
) error {
	...
	return rejectMigrationBackupNames(anchor, sidecars, names)
}

func rejectMigrationBackupNames(anchor *stateAnchor, sidecars bool, names []string) error {
	if anchor == nil {
		return ErrInvalidState
	}
...
func (anchor *stateAnchor) requireReaderValid(sidecars bool) error {
	if !anchor.readerValid(sidecars) {
		return ErrInvalidState
	}
