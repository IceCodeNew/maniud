...
func TestParseProjectsCompleteSupportedGrammar(t *testing.T) {
	...
	arguments := []string{
		...
		"-eFOO=bar", "--env=BARE", "--env-file=.env", "-lteam=platform", "-p127.0.0.1:8080:80/tcp",
	...
	if len(service.Mounts) != 4 || len(service.Ulimits) != 1 || len(service.Devices) != 1 ||
		len(service.Environment) != 2 || len(service.Ports) != 1 || !service.NoNewPrivileges ||
	...
	workload, err := projection.Workload(domain.ImageIdentity{
		Origin: domain.ImageOriginRegistry, Reference: "docker.io/team/app:1@" + digest.String(),
	...
	if workload.ServiceName != testService || workload.Ulimits[0] != (domain.Ulimit{
		Name: "nofile", Soft: 1024, Hard: 2048,
	}) || workload.Healthcheck.StartInterval != "500ms" {
