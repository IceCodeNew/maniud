...
func TestPortDeviceHostSysctlAndUlimitBoundaries(t *testing.T) {
	...
	validPorts := map[string]domain.PortBinding{
	...
	validDevices := map[string]domain.DeviceMapping{
...
func TestHealthAndParserInternalFailureBoundaries(t *testing.T) {
	...
	parser.service.Tmpfs = []domain.TmpfsMount{{Target: "/tmp", Options: []string{"ro"}}}
