...
// SupportedProviders returns the stable provider order used by configuration clients.
...
// ChoiceKind identifies one locally validated assistant response intent.
type ChoiceKind string

const (
	// ChoiceAnswer answers the user's request without proposing a file edit.
	ChoiceAnswer ChoiceKind = "answer"
	// ChoiceClarification asks the user for one additional input.
	ChoiceClarification ChoiceKind = "clarifying_question"
	// ChoiceRecommendation proposes one or more typed deployment changes.
	ChoiceRecommendation ChoiceKind = "recommendation"
)

// Choice is one provider response that passed JSON, schema, terminal-text,
// and, for recommendations, citation and typed deployment-value validation.
type Choice struct {
	Kind    ChoiceKind
	Message string
	Changes []Change
...
// Result is one normalized logical completion. Token is an opaque capability
// that accepts one choice from the current pending result.
type Result struct {
	Token          string
...
const (
	...
	ErrorConversationLimit ErrorCode = "llm_conversation_limit"
...
// ActionStage identifies the bounded stage that produced an LLM action error.
type ActionStage string

const (
	// ActionStageRequestPreparation fails before the provider transport starts.
	ActionStageRequestPreparation ActionStage = "request_preparation"
	// ActionStageProviderRequest fails while the provider request is in progress.
	ActionStageProviderRequest ActionStage = "provider_request"
	// ActionStageProviderResponse rejects a response returned by the provider.
	ActionStageProviderResponse ActionStage = "provider_response"
	// ActionStageContextValidation rejects a response after local context changed.
	ActionStageContextValidation ActionStage = "context_validation"
)

// RequestOutcome describes whether the last provider attempt was observed.
type RequestOutcome string

const (
	// RequestNotStarted proves that no provider transport attempt began.
	RequestNotStarted RequestOutcome = "request_not_started"
	// RequestOutcomeUnknown means an attempt began without a trusted response.
	RequestOutcomeUnknown RequestOutcome = "request_outcome_unknown"
	// RequestResponseReceived means the provider returned an HTTP response.
