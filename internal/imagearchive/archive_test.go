	...
	index := mustJSON(t, map[string]any{
		...
		"manifests": []map[string]any{{
			...
			"platform": map[string]any{
				"os": "linux", "architecture": platformArchitecture, "variant": options.variant,
			},
		}},
	})
...
func TestAnalyzeAcceptsLegacyLayerSymlink(t *testing.T) {
	...
	archivePath, _ := writeArchive(t, "", []tarMember{
		{name: manifestMember, body: manifest},
		{name: "c", body: config},
		{name: layerName, body: []byte("layer")},
...
func TestAnalyzeRejectsMalformedArchives(t *testing.T) {
	...
	valid := []tarMember{
		{name: manifestMember, body: manifest}, {name: "c", body: config}, {name: "l", body: []byte("x")},
	}
	tests := []struct {
		name     string
		members  []tarMember
