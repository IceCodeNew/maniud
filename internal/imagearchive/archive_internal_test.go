...
func TestScanArchiveAcceptsExactMemberLimit(t *testing.T) {
	t.Parallel()
	...
	if err := archive.WriteHeader(&tar.Header{
		Name: manifestName, Mode: 0o600, Size: int64(len(manifest)), Typeflag: tar.TypeReg, Format: tar.FormatUSTAR,
	}); err != nil {
		t.Fatal(err)
	}
	...
	file := mustBytesFile(t, raw.Bytes())
	defer closeTestFile(t, file)

	members, gotManifest, err := scanArchiveWithLimit(context.Background(), file, int64(raw.Len()), 1)
	if err != nil || len(members) != 1 || !bytes.Equal(gotManifest, manifest) {
		t.Fatalf("scanArchiveWithLimit(exact limit) = %#v, %q, %v", members, gotManifest, err)
	}
	if _, err = file.Seek(0, io.SeekStart); err != nil {
		t.Fatal(err)
	}
	if _, _, err = scanArchiveWithLimit(
		context.Background(), file, int64(raw.Len()), 0,
	); !errors.Is(err, ErrInvalidArchive) {
		t.Fatalf("scanArchiveWithLimit(over limit) error = %v", err)
	}
}
