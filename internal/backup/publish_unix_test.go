...
func TestPublishCreatesManifestLastAtomicDirectory(t *testing.T) {
	...
	root := privatePublicationRoot(t)
	manifest := validManifestForTest(t)
	result, err := publishCapacityChecked(context.Background(), t, root, manifest, publicationArchives(t, manifest, 512))
	if err != nil {
		t.Fatalf("Publish: %v", err)
	}
...
func assertPublicationReplay(t *testing.T, root string, manifest Manifest, result Publication) {
	...
	replayed, err := publishCapacityChecked(context.Background(), t, root, manifest, publicationArchives(t, manifest, 512))
...
func TestPublishRejectsDriftAndExistingConflict(t *testing.T) {
	...
	if !errors.Is(err, ErrPublicationConflict) {
		t.Fatalf("different raw artifact error = %v", err)
	}
