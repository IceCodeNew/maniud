...
func openPublication(
	...
	directory, _, openErr := openPublicationDirectory(root, transaction.String())
	if errors.Is(openErr, os.ErrNotExist) {
		return Publication{}, false, root.close()
	}
	if openErr != nil {
		return Publication{}, false, errors.Join(openErr, root.close())
	}
...
func verifyExistingPublication(
	...
	directory, identity, err := openPublicationDirectory(root, name)
	if err != nil {
		return false
	}
	defer func() {
...
func openPublicationDirectory(root *directoryAnchor, name string) (int, fileIdentity, error) {
	...
	if errors.Is(err, unix.ENOENT) {
		return -1, fileIdentity{}, os.ErrNotExist
	}
	if err != nil {
		return -1, fileIdentity{}, ErrInvalidArchive
	}
