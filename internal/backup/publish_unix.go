...
func (partial *partialPublication) stageArchives(
	...
	for index, input := range archives {
		expected := manifest.Artifacts[index].Inventory
		inventory, err := partial.writeArchive(ctx, artifactName(index), input, expected.ArchiveBytes)
		if err != nil {
			return err
		}
		if !SameContent(expected, inventory) {
...
func (partial *partialPublication) writeArchive(
	...
	maximumAnalyzeBytes := min(input.MaximumBytes, maximumWriteBytes)
	destination := &archiveLimitWriter{destination: file, remaining: maximumWriteBytes}
	inventory, analyzeErr := Analyze(ctx, io.TeeReader(input.Reader, destination), maximumAnalyzeBytes)
...
func (writer *archiveLimitWriter) Write(value []byte) (int, error) {
	allowed := min(int64(len(value)), writer.remaining)
	written, err := writer.destination.Write(value[:allowed])
	writer.remaining -= int64(written)
	if err != nil {
		return written, fmt.Errorf("write archive: %w", err)
	}
	if int64(written) != allowed {
		return written, io.ErrShortWrite
	}
	if allowed != int64(len(value)) {
		return written, ErrArchiveLimit
	}
