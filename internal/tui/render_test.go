...
func TestViewsMatchFullCompactHardAndResizeContracts(t *testing.T) {
	...
	state.timeline.entries = []timelineEntry{{outcome: observationCorrelated, stage: "postcondition_observed"}}
	content = state.View().Content
	assertViewContains(t, content, "compact constrained review", testCurrentLabel, testCurrentImage,
		testProposedLabel, testProposedImage, "Operation failed", "Continue to confirmation", "Operation failed.")
	if strings.Contains(content, statusReady) || strings.Contains(content, errTestSecret.Error()) {
		t.Fatalf("compact constrained review kept stale or unsafe copy: %q", content)
	}
}
...
func TestViewRendersEveryPageWithoutLeakingErrors(t *testing.T) {
	...
	pages := []struct {
		...
		{page: detailsPage{review: review}, contains: "Up/Down Scroll"},
...
func TestReviewAndDetailsRenderBoundedObservationSummary(t *testing.T) {
	...
	for _, value := range []string{
		"SESSION TIMELINE", "stage=action_completed", "Dropped events: 0", "Timeline truncated: yes", "x Export",
	} {
