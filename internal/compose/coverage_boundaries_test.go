...
func TestMaterializeRuntimeExactSnapshotAndFailClosed(t *testing.T) {
	...
	snapshot.Digest = repositoryDigest(snapshot.Entry, snapshot.Files, snapshot.RuntimePaths, nil)
	...
	for name, want := range files {
		...
		if readErr != nil || statErr != nil {
			t.Fatalf("materialized %s read/stat errors = %v/%v", name, readErr, statErr)
		}
		if !reflect.DeepEqual(got, want.Content) || info.Mode().Perm() != mode {
			t.Fatalf("materialized %s = %q mode %o", name, got, info.Mode().Perm())
		}
	}
	if err := pinned.MaterializeRuntime(); err != nil {
...
func TestRepositoryCollectorMalformedShapes(t *testing.T) {
	...
	if _, ok := repositoryEnvironment(RepositoryFile{Content: []byte("bad='")}); ok {
		t.Fatal("bad dotenv")
	}
