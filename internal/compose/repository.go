...
func collectResourceFiles(raw any, base string, result *[]repositoryDocument) bool {
	...
	for _, rawResource := range resources {
		...
		if file, found := resource["file"]; found && !collectPath(file, base, false, result) {
			return false
		}
	}
...
func collectExtends(raw any, base string, result *[]repositoryDocument) bool {
	...
	return collectPath(file, base, true, result)
}
	...
	for _, value := range values {
		if mapping, valid := value.(map[string]any); valid {
			for key := range mapping {
				if key != repositoryPathKey && key != "required" && key != "format" {
					return false
				}
			}
			value = mapping[repositoryPathKey]
		}
		if !collectPath(value, base, false, result) {
			return false
		}
	}
...
func collectPath(raw any, base string, compose bool, result *[]repositoryDocument) bool {
	path, valid := resolveRepositoryPath(raw, base)
	if !valid {
		return false
	}
	*result = append(*result, repositoryDocument{
		path: path, base: filepath.ToSlash(filepath.Dir(path)), compose: compose,
	})
