...
func CaptureRepositorySource(
	...
	resolvedEnvironment, valid := repositoryEnvironment(files[defaultEnvironment])
	if !valid {
		return Source{}, ErrInvalidSource
	}
	...
	return Source{
		...
		Repository: &RepositorySnapshot{
			...
			Digest: repositoryDigest(entry, files, runtimePaths, resolvedEnvironment),
		},
	}, nil
}
...
func repositoryEnvironment(file RepositoryFile) (map[string]string, bool) {
	if file.Content == nil {
		return map[string]string{}, true
	}
	result, err := dotenv.Parse(bytes.NewReader(file.Content))
	if err != nil {
		return nil, false
	}
...
func repositoryDigest(
	...
	environmentKeys := slices.Sorted(maps.Keys(environment))
	encoded = binary.AppendUvarint(encoded, uint64(len(environmentKeys)))
