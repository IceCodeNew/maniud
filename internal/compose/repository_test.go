...
func TestRepositoryDigestBindsEntryDocument(t *testing.T) {
	...
	first := repositoryDigest(testRootRepositoryEntry, files, nil)
	second := repositoryDigest("compose-alt.yaml", files, nil)
	if first == second {
...
func TestCaptureRepositorySourceLoadsTrackedProjectEnvironment(t *testing.T) {
	t.Parallel()
	...
	source, err := CaptureRepositorySource(
		t.TempDir(),
		testRootRepositoryEntry,
		map[string]string{"IMAGE": "example.com/api:2"},
		repositoryFixtureReader(files),
		func(string) (RepositoryPathSnapshot, error) {
			return RepositoryPathSnapshot{}, errRepositoryFixture
		},
	...
	if len(source.Repository.Files) != len(files) || source.Environment["IMAGE"] != "example.com/api:2" ||
		source.Environment["EXPANDED"] != "one-two" {
		t.Fatalf("CaptureRepositorySource() = %#v", source)
	}
}
