...
func TestDecodeManifestAcceptsDockerAndOCIIndexes(t *testing.T) {
	t.Parallel()

	for _, mediaType := range []string{dockerMediaTypeManifestList, ocispec.MediaTypeImageIndex} {
		raw := []byte(`{"schemaVersion":2,"manifests":[]}`)
		descriptorValue := descriptorForTest(raw, mediaType)

		document, err := decodeManifest(raw, descriptorValue)
		if err != nil || document.manifests == nil || document.digest.String() != descriptorValue.Digest.String() {
			t.Fatalf("decodeManifest(%q) = %#v, %v", mediaType, document, err)
		}
	}
}
...
func TestDecodeManifestRejectsInvalidDocuments(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:      "unsupported",
			raw:       `{}`,
			mediaType: testInvalidMediaType,
		},
	}
...
func TestDecodeImageConfigRejectsInvalidContent(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "unknown field",
			raw: `{"architecture":"amd64","os":"linux",` +
				`"rootfs":{"type":"layers","diff_ids":[]},"unknown":true}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name:      "unsupported media type",
			raw:       `{"architecture":"amd64","os":"linux","rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: testInvalidMediaType,
		},
		{
		...
		{
			name: "os version",
			raw: `{"architecture":"amd64","os":"linux","os.version":"1",` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "os features",
			raw: `{"architecture":"amd64","os":"linux","os.features":["x"],` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "process config shape",
			raw: `{"architecture":"amd64","os":"linux","config":[],` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "process arguments",
			raw: `{"architecture":"amd64","os":"linux","config":{"Entrypoint":1},` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "unknown process field",
			raw: `{"architecture":"amd64","os":"linux","config":{"Unknown":true},` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "duplicate process field",
			raw: `{"architecture":"amd64","os":"linux","config":{"Cmd":[],"Cmd":[]},` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			name: "NUL process argument",
			raw: `{"architecture":"amd64","os":"linux","config":{"Cmd":["bad\u0000value"]},` +
				`"rootfs":{"type":"layers","diff_ids":[]}}`,
			mediaType: ocispec.MediaTypeImageConfig,
		},
		{
			...
			raw: "{\"architecture\":\"amd64\",\"os\":\"linux\",\"config\":{\"Cmd\":[\"" +
				string([]byte{0xff}) + "\"]},\"rootfs\":{\"type\":\"layers\",\"diff_ids\":[]}}",
			mediaType: ocispec.MediaTypeImageConfig,
		},
	}
...
func TestDecodeImageConfigPreservesProcessDefaults(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			raw := []byte(`{"architecture":"amd64","os":"linux",` +
				`"rootfs":{"type":"layers","diff_ids":[]}` + test.config + `}`)
			...
			if err != nil || digest == (domain.Digest{}) ||
				(config.configuration.Entrypoint == nil) != (test.wantEntrypoint == nil) ||
				(config.configuration.Command == nil) != (test.wantCommand == nil) ||
				!slices.Equal(config.configuration.Entrypoint, test.wantEntrypoint) ||
				!slices.Equal(config.configuration.Command, test.wantCommand) {
