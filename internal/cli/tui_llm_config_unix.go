...
func readCurrentLLMEnv(workingDirectory string) (llmEnvState, string) {
	...
	state, reason := readLLMEnvEntry(root, false, defaultLLMConfigOperations())
...
func readXDGLLMEnv(environment map[string]string) (llmEnvState, string) {
	root, err := openLLMConfigRoot(environment, false, defaultLLMConfigOperations())
	...
	defer root.Close() //nolint:errcheck // A read-only diagnostic source has no pending write to settle.
	state, reason := readLLMEnvEntry(root, true, defaultLLMConfigOperations())
	if reason != "" || !state.exists {
...
func readLLMEnvEntry(
	...
	identity, reason := validateLLMEnvInfo(info, xdg)
	if reason != "" {
		return state, reason
	}
	...
	descriptorIdentity, descriptorReason := validateLLMEnvInfo(descriptorInfo, xdg)
	visibleIdentity, visibleReason := validateLLMEnvInfo(visible, xdg)
	closeErr := operations.closeFile(file)
	if readErr != nil || statErr != nil || closeErr != nil || visibleErr != nil ||
		descriptorReason != "" || visibleReason != "" || len(raw) > maximumLLMEnvBytes ||
		!operations.sameFile(info, descriptorInfo) || !operations.sameFile(descriptorInfo, visible) ||
		identity != descriptorIdentity || descriptorIdentity != visibleIdentity ||
		info.Mode().Perm() != descriptorInfo.Mode().Perm() || descriptorInfo.Mode().Perm() != visible.Mode().Perm() ||
		info.Size() != descriptorInfo.Size() || descriptorInfo.Size() != visible.Size() {
		return state, "entry changed while reading"
	}
	state.mode = descriptorInfo.Mode().Perm()
	state.identity = descriptorIdentity
...
//nolint:cyclop // This closed policy validates type, identity, permissions, and size together.
func validateLLMEnvInfo(info os.FileInfo, xdg bool) (llmFileIdentity, string) {
	if info == nil || !info.Mode().IsRegular() {
		return llmFileIdentity{}, "entry is not a regular file"
	}
	identity, valid := llmIdentity(info)
	if !valid || identity.links != 1 {
		return llmFileIdentity{}, "entry identity is unsafe"
	}
	permissions := info.Mode().Perm()
	if xdg {
		if identity.owner != effectiveUserID() || permissions != llmFileMode {
			return llmFileIdentity{}, "entry permissions are unsafe"
		}
	} else if identity.owner != 0 && identity.owner != effectiveUserID() || permissions&0o022 != 0 {
		return llmFileIdentity{}, "entry permissions are unsafe"
	}
	if info.Size() < 0 || info.Size() > maximumLLMEnvBytes {
		return llmFileIdentity{}, "entry is too large"
	}
...
func publishXDGLLMEnvWithOperations(
	...
	root, err := openLLMConfigRoot(environment, true, operations)
	if err != nil {
		return err
	}
	defer func() { returnErr = errors.Join(returnErr, operations.closeRoot(root)) }()
	lock, err := openLLMConfigLock(root, operations)
	if err != nil {
		return err
	}
	...
	current, reason := readLLMEnvEntry(root, true, operations)
	...
	published = true
	state, reason := readLLMEnvEntry(root, true, operations)
	if reason != "" || !state.exists || !bytes.Equal(state.raw, candidate) {
		return errLLMConfigSaveUnknown
	}
	if err = syncLLMConfigDirectory(root, operations); err != nil {
		if settleErr := syncLLMConfigDirectory(root, operations); settleErr != nil {
			return errLLMConfigSaveUnknown
		}
	}
...
func openLLMConfigLock(root *os.Root, operations llmConfigOperations) (*os.File, error) {
...
func syncLLMConfigDirectory(root *os.Root, operations llmConfigOperations) error {
