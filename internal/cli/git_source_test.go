...
type composeSourceFailureCase struct {
	name      string
	path      string
	configure func(*composeSourceFilesystem)
}

func TestLoadComposeSourceAcceptsLocalDayZeroFile(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	content := []byte("name: local\nservices: {}\n")
	writeGitSourceTestFile(t, root, composeFileValue, content, 0o600)

	source, err := loadComposeSource(t.Context(), composeFileValue, root, nil, t.TempDir())
	if err != nil || source.Repository != nil || source.WorkingDir != root ||
		string(source.Content) != string(content) || len(source.Environment) != 0 {
		t.Fatalf("loadComposeSource(local) = %#v, %v", source, err)
	}
}

func TestLoadComposeSourceDoesNotBypassDirtyRepository(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	writeGitSourceTestFile(t, root, composeFileValue, []byte("services: {}\n"), 0o600)
	commitApplyTestRepository(t, root, composeFileValue)
	writeGitSourceTestFile(t, root, "dirty", []byte("x"), 0o600)

	_, err := loadComposeSource(t.Context(), composeFileValue, root, nil, t.TempDir())
	if !errors.Is(err, compose.ErrInvalidSource) {
		t.Fatalf("loadComposeSource(dirty repository) = %v", err)
	}
}

func TestLoadComposeSourceRejectsUnsafeLocalPaths(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	content := []byte("services: {}\n")
	filesystem, _ := composeSourceFilesystemFixture(t, root, content)
	testLoadComposeSourceFailures(t, root, filesystem, []composeSourceFailureCase{
		{name: "invalid path", path: "bad\x00path"},
		{
			name: "metadata error", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.hasGitMetadata = func(string) (bool, error) { return false, io.ErrClosedPipe }
			},
		},
		{
			name: "symlink resolution error", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.evalSymlinks = func(string) (string, error) { return "", io.ErrClosedPipe }
			},
		},
		{
			name: "symlink", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.evalSymlinks = func(string) (string, error) {
					return filepath.Join(root, "resolved.yaml"), nil
				}
			},
		},
	})
...
func TestLoadComposeSourceRejectsUnsafeLocalContent(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	content := []byte("services: {}\n")
	filesystem, directory := composeSourceFilesystemFixture(t, root, content)
	testLoadComposeSourceFailures(t, root, filesystem, []composeSourceFailureCase{
		{
			name: "stat error", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.stat = func(string) (os.FileInfo, error) { return nil, io.ErrClosedPipe }
			},
		},
		{
			name: "non-regular file", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.stat = func(string) (os.FileInfo, error) { return directory, nil }
			},
		},
		{
			name: "read error", path: composeFileValue,
			configure: func(filesystem *composeSourceFilesystem) {
				filesystem.readFile = func(string) ([]byte, error) { return nil, io.ErrClosedPipe }
			},
		},
	})
}

func composeSourceFilesystemFixture(
	t *testing.T,
	root string,
	content []byte,
) (composeSourceFilesystem, os.FileInfo) {
	t.Helper()

	path := filepath.Join(root, composeFileValue)
	writeGitSourceTestFile(t, root, composeFileValue, content, 0o600)
	...
	return composeSourceFilesystem{
		hasGitMetadata: func(string) (bool, error) { return false, nil },
		evalSymlinks:   func(value string) (string, error) { return value, nil },
		stat:           func(string) (os.FileInfo, error) { return regular, nil },
		readFile:       func(string) ([]byte, error) { return content, nil },
	}, directory
}

func testLoadComposeSourceFailures(
	t *testing.T,
	root string,
	validFilesystem composeSourceFilesystem,
	tests []composeSourceFailureCase,
) {
	t.Helper()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := loadComposeSourceWithFilesystem(
				t.Context(), test.path, root, nil, t.TempDir(), filesystem,
			)
			if !errors.Is(err, compose.ErrInvalidSource) {
				t.Fatalf("loadComposeSourceWithFilesystem() error = %v", err)
			}
		})
	}
}

func TestHasGitMetadataBoundaries(t *testing.T) {
	t.Parallel()

	metadata, err := os.Stat(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	calls := 0
	found, err := hasGitMetadataWith(filepath.Join(string(filepath.Separator), "root", "child"), func(
		string,
	) (os.FileInfo, error) {
		calls++
		if calls == 1 {
	...
	if err != nil || !found || calls != 2 {
		t.Fatalf("hasGitMetadataWith(found) = %t, %v, calls %d", found, err, calls)
	}
	_, err = hasGitMetadataWith(t.TempDir(), func(string) (os.FileInfo, error) {
		return nil, io.ErrClosedPipe
	})
	if !errors.Is(err, io.ErrClosedPipe) {
		t.Fatalf("hasGitMetadataWith(error) = %v", err)
	}
}
