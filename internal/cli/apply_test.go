...
func (runtime *applyRuntimeFixture) Inspect(context.Context) (application.RuntimeEvidence, error) {
	*runtime.events = append(*runtime.events, inspectEvent)
	...
	return application.RuntimeEvidence{
		...
		Platform: domain.Platform{OS: linuxOS, Architecture: "amd64", Variant: ""},
...
func TestExecuteDryRunEmitsPlanAfterClosingEvidence(t *testing.T) {
	...
	valid := got.Project == "example" && got.Service == applyServiceValue && got.Status == "bootstrap" &&
		got.Runtime == "docker" && got.Platform == "linux/amd64" &&
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(dependencies *applyDependencies, _ *applyReaderFixture, _ *applyRuntimeFixture) {
				dependencies.loadSource = func(string) (compose.Source, error) {
		...
		},
		{
			...
			mutate: func(dependencies *applyDependencies, reader *applyReaderFixture, _ *applyRuntimeFixture) {
				...
				dependencies.openRuntime = func(context.Context) (applyRuntime, error) {
					return nil, errApplyTest
				}
			},
...
func TestLoadComposeSourcePinsPrivateBoundedBytes(t *testing.T) {
	t.Parallel()
	...
	source, err := loadComposeSource(composeFileValue, directory, environment)
	if err != nil {
		t.Fatalf("loadComposeSource() error = %v", err)
	}

	environment["VALUE"] = "changed"

	if !bytes.Equal(source.Content, content) || source.WorkingDir != directory ||
		source.Environment["VALUE"] != "private" || source.Profiles != nil {
		t.Fatalf("loadComposeSource() = %#v", source)
	}
}

func TestLoadComposeSourceRejectsUnsafeInputs(t *testing.T) {
	t.Parallel()

	directory := t.TempDir()
	regular := filepath.Join(directory, "regular.yaml")
	empty := filepath.Join(directory, "empty.yaml")
	large := filepath.Join(directory, "large.yaml")
	symlink := filepath.Join(directory, "link.yaml")

	err := os.WriteFile(regular, []byte("services: {}\n"), 0o600)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = os.WriteFile(large, bytes.Repeat([]byte("x"), maximumComposeSourceBytes+1), 0o600)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{path: "bad\x00path", workingDirectory: directory},
		{path: regular, workingDirectory: "relative"},
		{path: "missing.yaml", workingDirectory: directory},
		{path: ".", workingDirectory: directory},
		{path: empty, workingDirectory: directory},
		{path: large, workingDirectory: directory},
		{path: symlink, workingDirectory: directory},
	}

	for _, test := range tests {
		_, err := loadComposeSource(test.path, test.workingDirectory, nil)
		if !errors.Is(err, compose.ErrInvalidSource) {
			t.Fatalf("loadComposeSource(%q, %q) error = %v", test.path, test.workingDirectory, err)
		}
	}
}

type composeSourceFileFixture struct {
	reader   io.Reader
	info     os.FileInfo
	statErr  error
	closeErr error
...
func (file composeSourceFileFixture) Read(value []byte) (int, error) {
	// Preserve io.EOF exactly so the fixture follows the io.Reader contract.
	return file.reader.Read(value) //nolint:wrapcheck // A transparent reader fixture must return the delegate result.
}

func (file composeSourceFileFixture) Stat() (os.FileInfo, error) {
	return file.info, file.statErr
}

func (file composeSourceFileFixture) Close() error {
	return file.closeErr
}

type failingReaderWithError struct {
	err error
}

func (reader failingReaderWithError) Read([]byte) (int, error) {
	return 0, reader.err
}

//nolint:funlen // The table keeps the source identity race cases in one audit surface.
func TestLoadComposeSourceContainsFilesystemRaces(t *testing.T) {
	t.Parallel()
	...
	err = os.WriteFile(otherPath, []byte(content), 0o600)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		name       string
		filesystem func() composeSourceFilesystem
		want       error
	}{
		{
			name: "open",
			filesystem: func() composeSourceFilesystem {
				return composeSourceFilesystem{
					lstat: func(string) (os.FileInfo, error) { return pathInfo, nil },
					open:  func(string) (composeSourceFile, error) { return nil, errApplyTest },
				}
			},
		...
		},
		{
			...
			filesystem: func() composeSourceFilesystem {
				return fixedComposeFilesystem(pathInfo, composeSourceFileFixture{
					reader: strings.NewReader(content), info: nil, statErr: errApplyTest, closeErr: nil,
				})
			},
			want: compose.ErrInvalidSource,
		},
		{
			...
			filesystem: func() composeSourceFilesystem {
				return fixedComposeFilesystem(pathInfo, composeSourceFileFixture{
					reader: strings.NewReader(content), info: otherInfo, statErr: nil, closeErr: nil,
				})
			},
			want: compose.ErrInvalidSource,
		},
		{
			...
			filesystem: func() composeSourceFilesystem {
				return fixedComposeFilesystem(pathInfo, composeSourceFileFixture{
					reader: failingReaderWithError{err: errApplyTest}, info: pathInfo, statErr: nil, closeErr: nil,
				})
			},
			want: compose.ErrInvalidSource,
		},
		{
			name: "replaced after read",
			filesystem: func() composeSourceFilesystem {
				calls := 0
				...
				return composeSourceFilesystem{
					...
					open: func(string) (composeSourceFile, error) {
						return composeSourceFileFixture{
							reader: strings.NewReader(content), info: pathInfo, statErr: nil, closeErr: nil,
						}, nil
					},
				}
			},
		...
		},
		{
			...
			filesystem: func() composeSourceFilesystem {
				return fixedComposeFilesystem(pathInfo, composeSourceFileFixture{
					reader: strings.NewReader(content), info: pathInfo, statErr: nil, closeErr: errApplyCloseTest,
				})
			},
			want: errApplyCloseTest,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := loadComposeSourceWithFilesystem(path, directory, nil, test.filesystem())
			if !errors.Is(err, test.want) {
				t.Fatalf("loadComposeSourceWithFilesystem() error = %v, want %v", err, test.want)
			}
		})
	}
}

func fixedComposeFilesystem(info os.FileInfo, file composeSourceFile) composeSourceFilesystem {
	return composeSourceFilesystem{
		lstat: func(string) (os.FileInfo, error) { return info, nil },
		open:  func(string) (composeSourceFile, error) { return file, nil },
	}
}

//nolint:cyclop // The assertions audit each default factory and its construction failures together.
func TestDefaultApplyDependenciesOwnLifecycleFactories(t *testing.T) {
	...
	source, err := dependencies.loadSource(composePath)
	...
	runtime, err := dependencies.openRuntime(context.Background())
...
func TestDefaultApplyDependenciesOpenRuntime(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/v1.54/version":
			...
			_, _ = io.WriteString(response, `{"Version":"29.0.0","ApiVersion":"1.54",`+
				`"MinAPIVersion":"1.54","Os":"`+linuxOS+`","Arch":"amd64"}`)
		default:
	...
	runtimeAdapter, err := dependencies.openRuntime(context.Background())
	if err != nil {
...
func TestDefaultApplyDependenciesRejectsMissingWorkingDirectory(t *testing.T) {
	...
	if err == nil || dependencies.loadSource != nil || dependencies.openRuntime != nil ||
		dependencies.openReader != nil || dependencies.images != nil {
		t.Fatalf("defaultApplyDependencies() = (%+v, %v), want empty dependencies and error", dependencies, err)
	}
}

func TestRunDispatchesDryRunAndMapsFailure(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			execute: func(arguments applyInvocation) error {
				if arguments.compose != composeFileValue || arguments.service != applyServiceValue || !arguments.dryRun {
					return errApplyTest
				}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			status := run(
				context.Background(),
				[]string{string(commandApply), composeFileValue, applyServiceValue, dryRunOption},
				output,
				test.execute,
			)
...
func TestRunBuildsDefaultDryRunDependencies(t *testing.T) {
	...
	status = Run(
		context.Background(),
		[]string{string(commandApply), "compose.yaml", dryRunOption},
		nil,
		output,
		io.Discard,
	)
...
func TestPlatformString(t *testing.T) {
	...
	if got := platformString(domain.Platform{OS: linuxOS, Architecture: "amd64", Variant: ""}); got != "linux/amd64" {
