...
func TestLLMConfigurationFilesystemBoundaries(t *testing.T) {
	...
	t.Run("unsafe roots and locks", func(t *testing.T) {
		...
		if _, err := openLLMConfigRoot(
			map[string]string{xdgConfigHomeEnvironment: xdg}, true, defaultLLMConfigOperations(),
		); err == nil {
		...
		if _, err = openLLMConfigLock(root, defaultLLMConfigOperations()); !errors.Is(
			err, errLLMConfigPathInvalid,
		) {
			t.Fatalf("unsafe lock error = %v", err)
		}
		_ = root.Close()
		if err = syncLLMConfigDirectory(root, defaultLLMConfigOperations()); err == nil {
	...
	t.Run("final directory permissions", func(t *testing.T) {
		...
		if _, err := openLLMConfigRoot(
			map[string]string{xdgConfigHomeEnvironment: xdg}, false, defaultLLMConfigOperations(),
		); !errors.Is(
			err, errLLMConfigPathInvalid,
		) {
	...
	t.Run("current file unsafe permissions", func(t *testing.T) {
		...
		if _, reason := readLLMEnvEntry(
			root, false, defaultLLMConfigOperations(),
		); reason != "entry permissions are unsafe" {
...
func TestLLMConfigurationInjectedReadAndRootFaults(t *testing.T) {
	...
	for index, mutate := range readFaults {
		...
		mutate(&operations)
		if _, reason := readLLMEnvEntry(root, true, operations); reason == "" {
	...
	for index, mutate := range rootFaults {
		operations := defaultLLMConfigOperations()
		mutate(&operations)
		if opened, openErr := openLLMConfigRoot(environment, false, operations); openErr == nil {
			_ = opened.Close()
	...
	if _, err = openLLMConfigRoot(missingEnvironment, true, operations); err == nil {
		t.Fatal("mkdir fault succeeded")
	}
}

func TestLLMConfigurationReadRetainsDescriptorThroughIdentityCheck(t *testing.T) {
	t.Parallel()
	directory := t.TempDir()
	writeTestLLMEnv(t, filepath.Join(directory, llmConfigName), llmFileMode, "A=1\n")
	root, err := os.OpenRoot(directory)
	...
	if reason != "" || !state.valid || !closed || lstatCalls != 2 {
		t.Fatalf("read state = %#v, reason = %q, closed = %t, lstat calls = %d", state, reason, closed, lstatCalls)
	}
}

func TestLLMConfigurationReadRejectsMetadataChanges(t *testing.T) {
	t.Parallel()

	tests := map[string]func(string) error{
		"mode": func(path string) error {
			return os.Chmod(path, 0o644) //nolint:gosec // The test deliberately makes the secret file public.
		},
		"link count": func(path string) error {
	...
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			...
			directory := t.TempDir()
			path := filepath.Join(directory, llmConfigName)
			writeTestLLMEnv(t, path, llmFileMode, "A=1\n")
			root, err := os.OpenRoot(directory)
			if err != nil {
			...
			state, reason := readLLMEnvEntry(root, true, operations)
			if reason != "entry changed while reading" || state.valid {
				t.Fatalf("read metadata drift state = %#v, reason = %q", state, reason)
			}
		})
	}
}
