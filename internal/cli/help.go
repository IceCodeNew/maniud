...
func requestedHelp(args []string) (string, bool) {
	...
	if len(args) < minimumCommandHelpArguments {
		return "", false
	}
	helpArguments := args[1:]
	if args[0] == string(commandGen) {
		if separator := slices.Index(helpArguments, runtimeArgumentsSeparator); separator >= 0 {
			helpArguments = helpArguments[:separator]
		}
	}
	if !containsHelp(helpArguments) {
		return "", false
	}
