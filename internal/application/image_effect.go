...
func runImagePull(
	...
	intent := imagePullIntent(sequence, expected)
...
func imagePullIntent(sequence int64, expected domain.ImageIdentity) store.ActionIntent {
...
func validImageEffectIdentity(image domain.ImageIdentity) bool {
	return image.Origin == domain.ImageOriginRegistry && validRuntimeImageIdentity(image)
}

func validRuntimeImageIdentity(image domain.ImageIdentity) bool {
	if !completeRuntimeImageIdentity(image) {
		return false
	}
	...
	if image.Origin == domain.ImageOriginDockerArchive {
		return !source.IsPinned()
	}

	reference, parseErr := imageref.Parse(image.Reference)

	return image.Origin == domain.ImageOriginRegistry && parseErr == nil &&
		reference.Digest() == image.ReferenceDigest
}

func completeRuntimeImageIdentity(image domain.ImageIdentity) bool {
	empty := domain.Digest{}

	return image.ReferenceDigest != empty && image.PlatformManifest != empty && image.ImageConfig != empty &&
