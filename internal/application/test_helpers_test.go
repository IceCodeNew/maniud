...
const (
	...
	testProbeErrorName    = "probe error"
	testProbeFailureName  = "probe failure"
	testMissingProofName  = "missing with evidence"
	testSequenceName      = "sequence"
	testRuntimeName       = "runtime"
...
func (transactions testTransactions) AppliedService(
	ctx context.Context,
	project string,
	service string,
) (store.AppliedService, bool, error) {
	return transactions.applied(ctx, project, service)
}
...
func newTestTransactions(events *[]string) *testTransactions {
	return &testTransactions{
		...
		applied: func(_ context.Context, project, service string) (store.AppliedService, bool, error) {
			*events = append(*events, eventApplied)
...
func emptyImageIdentity() domain.ImageIdentity {
	return domain.ImageIdentity{
		Origin:           domain.ImageOriginUnknown,
		Reference:        "",
...
func appliedServiceForObservation(
	workload domain.DesiredWorkload,
	execution RuntimeEvidence,
	observation WorkloadObservation,
) store.AppliedService {
	return store.AppliedService{
		TransactionID:          store.TransactionID{1},
		Kind:                   store.TransactionBootstrap,
		Runtime:                execution.Kind,
		SourceDigest:           workload.SourceDigest,
		EffectiveDigest:        observation.Ownership.DesiredState,
		...
		ReferenceDigest:        observation.Ownership.Reference,
		PlatformManifestDigest: observation.Ownership.PlatformManifest,
		ImageConfigDigest:      observation.Ownership.ImageConfig,
	}
}
