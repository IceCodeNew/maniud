...
func TestRunImagePullRejectsInvalidExecutionInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: testSequenceName, journal: journal, sequence: 0,
			expected: expected, runtime: runtime, authenticator: authenticator,
		},
		{
			name:          "image",
			journal:       journal,
		...
		{
			name: testRuntimeName, journal: journal, sequence: 1,
			expected: expected, runtime: nil, authenticator: authenticator,
		},
