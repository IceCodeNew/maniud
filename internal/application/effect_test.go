...
type recoveredRuntimeEffect struct {
	events  *[]string
	applied bool
}

func (effect *recoveredRuntimeEffect) Apply(context.Context) error {
	*effect.events = append(*effect.events, eventEffect)
	effect.applied = true
...
func (effect *recoveredRuntimeEffect) Probe(context.Context) (EffectPostcondition, error) {
	*effect.events = append(*effect.events, eventProbe)

	return EffectPostcondition{
		Digest:    domain.Hash([]byte("recovered postcondition")),
		Satisfied: effect.applied,
	}, nil
}
...
func TestRunRuntimeEffectReplaysProvenMissingUnknownEffect(t *testing.T) {
	t.Parallel()

	journal, _, identifier, intent := newRuntimeEffectTest(store.ActionStateEffectOutcomeUnknown)
	effect := &recoveredRuntimeEffect{events: &journal.events}

	got, err := runRuntimeEffect(context.Background(), journal, identifier, intent, effect)
	if err != nil || !got.Satisfied || journal.action.State != store.ActionStateCompleted {
		t.Fatalf("runRuntimeEffect(recover missing) = %#v, %v, action %#v", got, err, journal.action)
	}
	wantEvents := []string{eventIntent, eventProbe, eventFence, eventEffect, eventProbe, eventComplete}
	if !equalEvents(journal.events, wantEvents) {
		t.Fatalf("events = %q, want %q", journal.events, wantEvents)
	}
}

func TestRunRuntimeEffectDoesNotReplayWithoutRecoveryFence(t *testing.T) {
	t.Parallel()

	journal, _, identifier, intent := newRuntimeEffectTest(store.ActionStateEffectOutcomeUnknown)
	journal.failures[eventFence] = errTestBoundary
	effect := &recoveredRuntimeEffect{events: &journal.events}

	got, err := runRuntimeEffect(context.Background(), journal, identifier, intent, effect)
	if !errors.Is(err, errTestBoundary) || got != emptyEffectPostcondition() || effect.applied ||
		journal.action.State != store.ActionStateEffectOutcomeUnknown {
		t.Fatalf("runRuntimeEffect(recovery fence) = %#v, %v, effect %#v, action %#v", got, err, effect, journal.action)
	}
	wantEvents := []string{eventIntent, eventProbe, eventFence}
	if !equalEvents(journal.events, wantEvents) {
		t.Fatalf("events = %q, want %q", journal.events, wantEvents)
	}
}
...
func TestRunRuntimeEffectContainsBoundaryFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.failureAt == eventFence {
				wantEvents := []string{eventIntent, eventFence}
				if !equalEvents(journal.events, wantEvents) ||
					journal.action.State != store.ActionStateIntent {
					t.Fatalf("fence failure = events %q, state %q", journal.events, journal.action.State)
				}
			}
		})
	}
...
func TestRunRuntimeEffectContainsRecoveryProbeFailures(t *testing.T) {
	t.Parallel()

	for _, test := range []struct {
		name   string
		mutate func(*testRuntimeEffect)
		want   error
	}{
		{name: "probe", mutate: func(effect *testRuntimeEffect) {
			effect.probeErr = context.Canceled
		}, want: context.Canceled},
		{name: "empty postcondition", mutate: func(effect *testRuntimeEffect) {
			effect.postcondition.Digest = domain.Digest{}
		}, want: ErrConflictingState},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			journal, effect, identifier, intent := newRuntimeEffectTest(store.ActionStateEffectOutcomeUnknown)
			test.mutate(&effect)

			got, err := runRuntimeEffect(context.Background(), journal, identifier, intent, effect)
			if !errors.Is(err, test.want) || got != emptyEffectPostcondition() {
				t.Fatalf("runRuntimeEffect(recovery %s) = %#v, %v", test.name, got, err)
			}
		})
	}
}
...
func TestRunRuntimeEffectRejectsInvalidDependenciesAndJournalEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: testInvalidStateName, state: testInvalidValue, mutate: func(*testEffectJournal) {}},
...
func newEffectEvents() []string {
	return []string{eventIntent, eventFence, eventUnknown, eventEffect, eventProbe, eventComplete}
}
