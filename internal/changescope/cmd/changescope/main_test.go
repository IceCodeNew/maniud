...
//nolint:paralleltest // This test owns the process-global execution seams.
func TestMainReportsExecuteStatus(t *testing.T) {
	...
	main()
	if got != want {
		t.Fatalf("main() exit = %d, want %d", got, want)
	}
}
...
func TestRunMainUsesProcessArguments(t *testing.T) {
	...
	os.Stderr = stderr
	if status := runMain(); status != statusUsage {
		t.Fatalf("runMain() = %d, want %d", status, statusUsage)
	}
}
...
//nolint:paralleltest,tparallel // The table subtests run in parallel after the parent returns.
func TestExecuteReportsFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing output", want: statusUsage},
		{
		...
		{name: "missing affected input", args: []string{testOutputFlag, testManifest}, want: statusUsage},
		{
			name: "read paths",
			args: []string{testBaseFlag, testBase, testPathsFlag, testPaths, testOutputFlag, testManifest},
			edit: func(deps *dependencies) {
