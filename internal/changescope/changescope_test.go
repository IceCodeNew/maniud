...
func TestSelectExpandsModuleInputsToTheirWholeModule(t *testing.T) {
	repository := newRepository(t)
	write(t, repository, "go.mod", "module example.test/root\n\ngo 1.27\n")
	write(t, repository, "go.sum", "")
	write(t, repository, "root.go", "package root\n")
	write(t, repository, "other/other.go", "package other\n")
	write(t, repository, "nested/go.mod", "module example.test/nested\n\ngo 1.27\n")
	write(t, repository, "nested/go.sum", "")
	write(t, repository, "nested/nested.go", "package nested\n")
	write(t, repository, "nested/other/other.go", "package other\n")
	revision := commit(t, repository, "base")
	...
	tests := []struct {
		...
		{path: "go.mod", module: ".", packages: "example.test/root,example.test/root/other"},
		{path: "go.sum", module: ".", packages: "example.test/root,example.test/root/other"},
		{path: "nested/go.mod", module: testNestedModule, packages: "example.test/nested,example.test/nested/other"},
		{path: "nested/go.sum", module: testNestedModule, packages: "example.test/nested,example.test/nested/other"},
	}
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			manifest, err := Select(repository, revision, revision, []string{test.path})
			if err != nil {
				t.Fatal(err)
			}
			if got := strings.Join(manifest.Modules, ","); got != test.module {
				t.Fatalf("modules = %q, want %q", got, test.module)
			}
			if got := strings.Join(manifest.Packages[test.module], ","); got != test.packages {
				t.Fatalf("packages = %q, want %q", got, test.packages)
			}
		})
	}
}
...
func TestSelectKeepsPackagesRemovedWithTheirModule(t *testing.T) {
	...
	manifest, err := Select(repository, base, head, []string{"old/go.mod", "old/old.go"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoaderReportsFilesystemAndPackageFailures(t *testing.T) {
	...
	t.Run("module read", func(t *testing.T) {
		...
		//nolint:gosec // The test reads its own fixture to detect permission bypass.
...
func TestPackageDecoderRejectsUnusableDirectory(t *testing.T) {
	graph := emptySide(t.TempDir())
	relative := "{\"ImportPath\":\"relative\",\"Dir\":\"relative\"}\n"
	if err := decodePackages(graph, []byte(relative)); !errors.Is(err, errPackageOutside) {
		t.Fatalf("decodePackages() relative directory error = %v", err)
	}
}
...
func TestExtractRevisionReportsProcessFailures(t *testing.T) {
	...
	t.Run("pipe", func(t *testing.T) {
		archive := exec.CommandContext(t.Context(), "git", "version")
		archive.Stdout = &bytes.Buffer{}
		extract := exec.CommandContext(t.Context(), "tar", "--version")
		if err := runArchiveExtraction(archive, extract); err == nil {
			t.Fatal("runArchiveExtraction() accepted a configured archive output")
		}
	})
...
func TestOwnershipHelpersRejectMissingOwners(t *testing.T) {
	for _, path := range []string{
		"outside/file.go",
		filepath.Join(string(filepath.Separator), "outside", "file.go"),
	} {
		if _, ok := owningKey(path, map[string]string{testNestedModule: "module"}); ok {
			t.Fatalf("owningKey(%q) found an unrelated module", path)
		}
	}
