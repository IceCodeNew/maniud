...
func TestBootstrapGuardWidensUnsafeChanges(t *testing.T) {
	...
	tests := []struct {
		...
		{"tabbed-build-tag", func() { write(t, repository, "tagged.go", "//go:build\tdarwin\n\npackage root\n") }},
		{"legacy-build-tag", func() { write(t, repository, "tagged.go", "// +build darwin\n\npackage root\n") }},
		{"delete", func() {
	...
	t.Run("missing-head", func(t *testing.T) {
		invokeBootstrap(t, repository, base, strings.Repeat("f", 40), testFullMode)
	})
}
...
func TestFullManifestUsesHeadSnapshot(t *testing.T) {
	...
	base := commit(t, repository, "base")
	if err := os.Remove(filepath.Join(repository, "internal/dirty/dirty.go")); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{name: "explicit head", args: []string{"--full", "--head", "HEAD"}},
		{
			name: "invalid head fallback",
			args: []string{"--base", base, "--head", strings.Repeat("f", 40)},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			manifest := filepath.Join(t.TempDir(), "manifest.tsv")
			args := slices.Concat(test.args, []string{"--output", manifest})
			run(t, repository, "bash", append([]string{"scripts/select-affected-gates"}, args...)...)
			contents, err := os.ReadFile(manifest) //nolint:gosec // The path belongs to this test.
			if err != nil {
				t.Fatal(err)
			}
			if !strings.Contains(string(contents), "package\t.\tgithub.com/IceCodeNew/maniud/internal/dirty\n") {
				t.Fatalf("full manifest read the dirty working tree:\n%s", contents)
			}
		})
	}
}

func TestLocalFullManifestUsesIndexSnapshot(t *testing.T) {
	repository := bootstrapRepository(t)
	...
	if err := os.Remove(filepath.Join(repository, "nested/nested.go")); err != nil {
		t.Fatal(err)
	}
...
func TestCoverageGatePropagatesPackageSelectionFailure(t *testing.T) {
	...
	manifest := filepath.Join(repository, "manifest.tsv")
	write(t, repository, filepath.Base(manifest),
...
`)
	...
	command := exec.CommandContext(t.Context(), "bash", "scripts/check-go-coverage")
	command.Dir = repository
	command.Env = append(
		os.Environ(),
		"MANIUD_GATE_MANIFEST="+manifest,
		"PATH="+bin+string(os.PathListSeparator)+os.Getenv("PATH"),
	)
	output, err := command.CombinedOutput()
	if err == nil || !strings.Contains(string(output), "injected go list failure") {
		t.Fatalf("check-go-coverage did not propagate go list failure: %v\n%s", err, output)
	}
}
