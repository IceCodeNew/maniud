...
func (selected *selection) classify(changedPaths []string, graphs []*side) error {
	for _, path := range changedPaths {
		...
		moduleInput := filename == "go.mod" || filename == "go.sum"
		classified := false
		for _, graph := range graphs {
			if moduleInput {
				if module, ok := owningKey(path, graph.modules); ok {
...
func extractRevision(repository, revision, destination string) error {
	...
	return runArchiveExtraction(archive, extract)
}

func runArchiveExtraction(archive, extract *exec.Cmd) error {
	pipe, err := archive.StdoutPipe()
	if err != nil {
		return fmt.Errorf("open archive output: %w", err)
	}
	extract.Stdin = pipe
...
func decodePackages(graph *side, output []byte) error {
	...
	for {
		...
		relative, err := filepath.Rel(graph.root, pkg.Dir)
		if err != nil {
			return fmt.Errorf("%w: %q: %w", errPackageOutside, pkg.Dir, err)
		}
