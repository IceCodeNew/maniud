...
func TestHostConfigurationContentAndNamespaceProjection(t *testing.T) {
	...
	emptyHosts, err := hostsContent("", nil)
	if emptyHosts != nil || err != nil {
		t.Fatal("hostsContent() returned content for an empty host set")
	}
	if _, err = hostsContent("", []string{testBadValue}); !errors.Is(err, ErrUnsupportedWorkload) {
		t.Fatal("hostsContent() accepted an invalid empty or extra host set")
	}
	hostContent, err := hostsContent(testWorkloadService, []string{"database=192.0.2.10"})
	if err != nil {
		t.Fatalf("hostsContent() error = %v", err)
	}
	hosts := string(hostContent)
...
//nolint:cyclop,funlen // The test exercises each private-file and host-absence failure boundary.
func TestPrivateFileAndHostAbsenceFailures(t *testing.T) {
	...
	if err = writePrivateFileContents(
		writer, []byte("value"), filepath.Join(t.TempDir(), testFileName),
	); !errors.Is(err, ErrUnavailable) {
		t.Fatalf("writePrivateFileContents(sync) = %v", err)
	}
	if _, err = writer.Stat(); !errors.Is(err, os.ErrClosed) {
		t.Fatalf("writePrivateFileContents(sync) left file open: %v", err)
	}
...
func TestHostMountComparisonAndGeneratedFileFailures(t *testing.T) {
	...
	if _, err := appendGeneratedHostMounts(testHostDirectory(t), nil, domain.WorkloadSpec{
		ExtraHosts: []string{testBadValue},
	}); !errors.Is(err, ErrUnsupportedWorkload) {
		t.Fatalf("appendGeneratedHostMounts(invalid host) = %v", err)
	}
	root := testHostDirectory(t)
