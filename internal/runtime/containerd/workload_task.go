...
func (backend *nativeWorkloadBackendV1) Start(ctx context.Context, identifier string) error {
	...
	if err = backend.host.EnsureNetworkNamespace(networkNamespace); err != nil {
...
func (backend *nativeWorkloadBackendV1) Remove(
	...
	if force && (workload.Lifecycle == application.WorkloadLifecycleRunning ||
		workload.Lifecycle == application.WorkloadLifecyclePaused) {
	...
	if _, err = backend.containers.Delete(ctx, &containersapi.DeleteContainerRequest{ID: identifier}); err != nil {
		return classifyRPCError(err)
	}
	...
	return wrapWorkloadBackendError(
		"workload host-state removal",
		backend.host.Remove(backend.options, identifier, workload.RuntimeMounts),
	)
}
