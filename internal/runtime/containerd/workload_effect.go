...
func (client *Client) probeRemovedWorkload(
	...
	err := client.checked(ctx, func(requestContext context.Context) error {
		var err error
		byID, err = client.workloads.Workload(requestContext, before.ID)
		if err != nil {
			return wrapWorkloadBackendError("removal ID probe", err)
		}
		available, err = client.workloads.NameAvailable(requestContext, before.Name, "")
		if err != nil || byID != nil || !available {
			return wrapWorkloadBackendError("removal name probe", err)
		}
		removalComplete, err = client.workloads.RemovalComplete(requestContext, before.ID)

		return wrapWorkloadBackendError("removal cleanup probe", err)
	})
	if err != nil {
		return application.WorkloadTransitionProbe{}, workloadError(err)
	}
	if byID == nil && available && removalComplete {
