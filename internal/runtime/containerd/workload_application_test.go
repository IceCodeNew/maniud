...
func TestWorkloadTransitionsApplyAndProbe(t *testing.T) {
	...
	if err := client.ResumeIncompleteWorkloadTransition(
		context.Background(), invalid,
	); !errors.Is(err, ErrUnsupportedWorkload) {
		t.Fatalf("ResumeIncompleteWorkloadTransition(invalid) = %v", err)
	}
}
...
func TestRemovalProbeRejectsIncompleteCleanup(t *testing.T) {
	...
	partial := *native
	partial.Lifecycle = application.WorkloadLifecycleCreated
	backend := &fakeWorkloadBackend{removalCandidate: &partial}
	client := testCheckedWorkloadClient(t, backend)
	probe, err := client.ProbeWorkloadTransition(context.Background(), transition)
	removing := transition.Before
	removing.Lifecycle = application.WorkloadLifecycleRemoving
	if err != nil || probe.State != application.WorkloadEffectProbeObserved || probe.Workload != removing {
		t.Fatalf("ProbeWorkloadTransition(partial removal) = %#v, %v", probe, err)
	}
	if err = client.ResumeIncompleteWorkloadTransition(context.Background(), transition); err != nil ||
		backend.removed != 1 {
		t.Fatalf("ResumeIncompleteWorkloadTransition() = %v, calls %d", err, backend.removed)
	}
	running := *native
	running.Lifecycle = application.WorkloadLifecycleRunning
	backend.removalCandidate = &running
	if err = client.ResumeIncompleteWorkloadTransition(
		context.Background(), transition,
	); !errors.Is(err, ErrProtocol) {
		t.Fatalf("ResumeIncompleteWorkloadTransition(running) = %v", err)
	}

	backend.removalCandidate = nil
	backend.available = true
	backend.removalIncomplete = true
	if _, err = client.ProbeWorkloadTransition(context.Background(), transition); !errors.Is(err, ErrProtocol) {
		t.Fatalf("ProbeWorkloadTransition(incomplete cleanup) = %v", err)
	}
	if err = client.ResumeIncompleteWorkloadTransition(
		context.Background(), transition,
	); !errors.Is(err, ErrProtocol) {
		t.Fatalf("ResumeIncompleteWorkloadTransition(unproven) = %v", err)
	}
}
...
func TestRemovalProbeReadFailureMatrix(t *testing.T) {
	...
	backend.available = true
	backend.removalCandidate = native
	probe, err := client.ProbeWorkloadTransition(context.Background(), transition)
	if err != nil || probe.State != application.WorkloadEffectProbeObserved {
