...
func TestProbeImageProvesResolvedPlatformIdentity(t *testing.T) {
	...
	client := connectedTestClient(t, http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		document := strings.Replace(
			imageInspectDocument(expected, true),
...
func TestHasReferenceDigestAcceptsDockerFamiliarName(t *testing.T) {
	...
	expected := testImageIdentity(t)
	reference, err := imageref.Parse("docker.io/library/api@" + expected.ReferenceDigest.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	if hasReferenceDigest([]string{"other@" + expected.ReferenceDigest.String()}, reference) {
		t.Fatal("hasReferenceDigest() accepted a different repository")
	}
}
...
func TestProbeImageRejectsConflictingEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: imagePullPlatformQuery, mutate: func(value string) string {
