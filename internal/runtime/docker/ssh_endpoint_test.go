...
func TestSSHHandshakeAndChannelCancellation(t *testing.T) {
	...
	for _, mode := range []sshServerMode{sshServerIgnoreChannel, sshServerIgnoreExec} {
		...
		endpoint, endpointErr := SSHEndpoint(
			server.endpointURL(),
			privateKeySSHOptions(privateKey, knownHosts),
		)
		if endpointErr != nil {
			t.Fatalf("SSHEndpoint(mode %d) error = %v", mode, endpointErr)
		}

		cancelContext, cancelDial := context.WithTimeout(context.Background(), 20*time.Millisecond)
		_, err := endpoint.transport.DialContext(cancelContext, "tcp", "ignored")
