...
func TestInspectRejectsUnsupportedDaemonPlatform(t *testing.T) {
	...
	client.version.Architecture = testUnsupportedArchitecture

	got, err := client.Inspect(context.Background())

	var empty application.RuntimeEvidence

	if !reflect.DeepEqual(got, empty) || !errors.Is(err, ErrUnsupportedWorkload) {
...
func TestInspectContainsDaemonFailure(t *testing.T) {
	...
	if !reflect.DeepEqual(got, empty) || !errors.Is(err, ErrUnavailable) {
	...
	return []applicationObservationTest{
		...
		},
		{
			...
			mutate: func(workload *domain.DesiredWorkload) {
				workload.Command = []string{testOtherValue}
			},
			wantState:  application.WorkloadObservationPresent,
			wantConfig: false,
			wantOwner:  domain.OwnershipManaged,
...
func TestObserveWorkloadContainsValidationAndProbeFailures(t *testing.T) {
	...
	for _, state := range []ContainerProbeState{ContainerProbeUnknown, ContainerProbeState(255)} {
		...
		if !reflect.DeepEqual(got, empty) || !errors.Is(err, ErrProtocol) {
			t.Fatalf("workloadObservation(%d) = %#v, %v", state, got, err)
		}
	}

	invalidStorage := validApplicationWorkload(t)
	invalidStorage.SourceDigest = domain.Digest{}
	got, err = workloadObservation(observedContainerProbe(t), invalidStorage)
	if !reflect.DeepEqual(got, empty) || !errors.Is(err, ErrProtocol) {
		t.Fatalf("workloadObservation(invalid storage) = %#v, %v", got, err)
	}
}
...
func unsupportedApplicationWorkloads(t *testing.T) []domain.DesiredWorkload {
	...
	mutations := []func(*domain.DesiredWorkload){
		...
		func(value *domain.DesiredWorkload) { value.Image.Origin = domain.ImageOriginUnknown },
		func(value *domain.DesiredWorkload) { value.Image.Origin = domain.ImageOrigin(255) },
...
func TestContainerConfigurationDigestPreservesNilSlices(t *testing.T) {
	t.Parallel()

	container := Container{
		Name:             "example-api",
		ImageReference:   "example.com/team/api@sha256:" + strings.Repeat("1", 64),
	...
	withNil := containerConfigurationDigest(container)
	container.WorkloadSpec.Entrypoint = []string{}
	withEmpty := containerConfigurationDigest(container)
	if withNil == withEmpty {
		t.Fatal("containerConfigurationDigest() collapsed nil and empty entrypoints")
	}
}
...
func TestDockerPlatformSupportsPhaseOneArchitectures(t *testing.T) {
	...
	tests := []struct {
		...
		{osName: dockerOperatingSystem, architecture: testUnsupportedArchitecture, want: empty, valid: false},
	}
