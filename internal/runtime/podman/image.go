...
func podmanImageEvidence(
	...
	manifest, manifestErr := domain.ParseDigest(payload.Digest)
	configObject := make(map[string]json.RawMessage)
	if !validPodmanImageCore(payload, expected, imageConfig, manifest, configValid, manifestErr) ||
	...
	return application.ImageEvidence{
		ReferenceDigest: expected.ReferenceDigest, PlatformManifest: manifest,
...
func validPodmanImageCore(
	...
	return configValid && manifestErr == nil && manifest == expected.PlatformManifest &&
