...
func TestPodmanWorkloadEncodingAddsOnlyOwnedLabels(t *testing.T) {
	...
	workload.Labels = []string{"com.example.role=api"}
	...
	if !ok || labels["com.example.role"] != "api" || labels[domain.LabelTransaction] != podmanTestTransaction {
...
func TestPodmanInspectionBindsProjectEvidence(t *testing.T) {
	...
	if !valid || container.State != ContainerRunning || container.Ownership.Status != domain.OwnershipManaged ||
		...
		!reflect.DeepEqual(container.WorkloadSpec.Labels, []string{"com.example.role=api"}) {
	...
	inspection.ImageID = podmanImageConfig
	inspection.ImageDigest = podmanManifestDigest
