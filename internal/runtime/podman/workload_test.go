...
func (state *podmanWorkloadRuntimeState) create(writer http.ResponseWriter, request *http.Request) {
	...
	if !decodePodmanJSON(request.Body, maximumControlBytes, &configuration) ||
		configuration.Name != state.workload.ContainerName ||
		configuration.Image != state.workload.Image.Reference || configuration.RestartPolicy != "no" ||
		configuration.NetworkNamespace.Mode != podmanNetworkBridge ||
...
func (state *podmanWorkloadRuntimeState) inspectDocument() string {
	...
	payload := podmanInspectData{
		...
		HostConfig: &podmanInspectHost{
			NetworkMode: podmanNetworkBridge, CgroupMode: podmanCgroupPrivate,
			Cgroups: podmanCgroupsEnabled, ShmSize: podmanDefaultSharedMemory,
...
func TestPodmanCanonicalOrderBranches(t *testing.T) {
	t.Parallel()

	spec := domain.WorkloadSpec{
		Tmpfs:   []domain.TmpfsMount{{Target: "/z"}, {Target: "/a"}},
		Ulimits: []domain.Ulimit{{Name: "z"}, {Name: "a"}},
		ExposedPorts: []domain.ExposedPort{
			{TargetPort: 81, Protocol: podmanProtocolTCP},
			{TargetPort: 80, Protocol: podmanProtocolUDP},
			{TargetPort: 80, Protocol: podmanProtocolTCP},
		},
		Ports: []domain.PortBinding{
			{TargetPort: 81, Protocol: podmanProtocolTCP},
			{TargetPort: 80, Protocol: podmanProtocolTCP},
		},
		Mounts: []domain.Mount{{Source: "/z", Target: "/z"}, {Source: "/a", Target: "/a"}},
	}
	canonicalPodmanOrder(&spec)
	if spec.Tmpfs[0].Target != "/a" || spec.Ulimits[0].Name != "a" ||
		spec.ExposedPorts[0].Protocol != podmanProtocolTCP || spec.ExposedPorts[2].TargetPort != 81 ||
		spec.Ports[0].TargetPort != 80 || spec.Mounts[0].Target != "/a" {
		t.Fatalf("canonicalPodmanOrder() = %#v", spec)
	}
}
...
func TestPodmanOwnershipBranches(t *testing.T) {
	...
	if ownership := decodeOwnership(nil, imageConfig, manifest); ownership.Status != domain.OwnershipUnmanaged {
	...
	if ownership := decodeOwnership(labels, imageConfig, manifest); ownership != (domain.WorkloadOwnership{}) {
...
func TestPodmanObservedWorkloadAndInspectMappingBranches(t *testing.T) {
	...
	payload := podmanRichInspect(t, podmanRichWorkload(t))
